...
abstract class BaseDatabaseConnectionOptions implements ConnectionOptions {
	...
	private static final Logger logger = LoggerFactory.getLogger(BaseDatabaseConnectionOptions.class);
	...
	protected BaseDatabaseConnectionOptions(final UserCredentials userCredentials, final Map<String, String> properties)
	...
	@Override
	public final Connection getConnection(final String user, final String password) throws SQLException {
		if (isBlank(user)) {
			logger.warn("Database user is not provided");
		}
		if (isBlank(password)) {
			logger.warn( "Database password is not provided");
		}
		...
		final Properties jdbcConnectionProperties = createConnectionProperties(connectionUrl, user, password);
		try {
			 logger.info("Making connection to {} for user \'{}\', with properties {}",connectionUrl,user,safeProperties(jdbcConnectionProperties));
			...
			final Connection connection = driver.connect(connectionUrl, jdbcConnectionProperties);
			 logger.info("Opened database connection <{}>",connection);
			...
			throw new DataPagerSQLException(String.format("Could not connect to %s, for %s, with properties %s",
					connectionUrl, username, safeProperties(jdbcConnectionProperties)), e);
		}
	}
	...
	public String getConnectionUrl() {
		...
		final Set<String> unmatchedVariables = TemplatingUtility.extractTemplateVariables(connectionUrl);
		if (!unmatchedVariables.isEmpty()) {
			throw new IllegalArgumentException(String
					.format("Insufficient parameters for database connection URL: missing %s", unmatchedVariables));
		}
	...
	public final String toString() {
		...
		builder.append("url=").append(getConnectionUrl()).append(System.lineSeparator());
	...
	private Properties createConnectionProperties(final String connectionUrl, final String user, final String password)
			throws SQLException {
		final List<String> skipProperties = Arrays.asList("server", "host", "port", "database", "urlx", "user",
				"password", "url");
	...
	private final Driver getJdbcDriver(final String connectionUrl) throws SQLException {
		try {
			return DriverManager.getDriver(connectionUrl);
		} catch (final SQLException e) {
			throw new DataPagerSQLException(
					"Could not find a suitable JDBC driver for database connection URL, " + getConnectionUrl(), e);
		}
	}

	private void logConnection(final Connection connection) {
		if (connection == null || !logger.isInfoEnabled()) {
			return;
		}
		try {
			final DatabaseMetaData dbMetaData = connection.getMetaData();
			
			logger.info("Connected to %n{} {} %nusing JDBC driver %n{} {}",
			...
			logger.error("Could not log connection information", e);
		}
	}
				...
				if (!isBlank(property)) {
					...
					if (propertyValues.length >= 2) {
						...
						if (key != null && value != null) {
							// Properties is based on Hashtable, which cannot
							// take
							// null keys or values
